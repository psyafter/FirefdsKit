...
import static sb.firefds.v.firefdskit.XSysUIGlobalActions.getActionViewModelDefaults;
import static sb.firefds.v.firefdskit.XSysUIGlobalActions.isUnlockKeyguardBeforeActionExecute;
import static sb.firefds.v.firefdskit.utils.Constants.TAG;
        ...
        if (!getGlobalActions().isActionConfirming()) {
            ...
            if (isUnlockKeyguardBeforeActionExecute()) {
                if (mConditionChecker.isEnabled(SystemConditions.IS_SECURE_KEYGUARD)) {
                    for (SecureConfirmStrategy strategy3 : mFeatureFactory.createSecureConfirmStrategy(getGlobalActions(),
